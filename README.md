...
```text
Interaction / Events
Chat · Voice · Email · Calendar · Home · Server
                     ↓
┌──────────────────────────────────────────────┐
...
│                                              │
│ Durable assets:                              │
│ Memory · Task · Capability · Policy · State  │
└──────────────────────────────────────────────┘
        ↓                  ↓                 ↓
    Runtime Layer      Memory Engines     Providers
...
1. Event / World State
2. Canonical Task / Semantic Checkpoint
3. Memory Contract / Memory Policy / Memory Broker
4. SRI + Hermes Adapter + DSH Adapter
5. Context Compiler
6. Capability Registry / Gateway / Execution Ledger
7. Policy / Approval / Idempotency
8. Scheduler / Pulse
9. PostgreSQL + pgvector
10. Minimal Web Console / CLI
...
- **Autonomous Event Handling** — 无用户 Prompt 时，事件可触发 Pulse → Task → Runtime → Capability。
- **Runtime Upgrade** — Candidate Runtime 经 replay / canary 后切换，Memory / Task / Capability / Policy 不迁移。
- **Task-aware Memory** — 相比普通 Vector Top-K，减少无关上下文并改善任务决策。
...
**Design baseline / pre-MVP.** 当前产品边界已经收敛，下一阶段进入 Contract、LLD、数据库 schema、API 与 MVP 实现。
